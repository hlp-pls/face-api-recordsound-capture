...
let capture_DOM = document.getElementById("capture");
...
function record(){
	record_count++;
	if(!is_recording){
		record_DOM.innerText = "Sound recording.... press again to stop.";
		is_recording = true;
	}else{
		record_DOM.innerText = "Press to record sound.";
		is_recording = false;
	}
}


capture_DOM.addEventListener("click",capture,false);
capture_DOM.addEventListener("touchend",capture,false);

function capture(){
	saveFrames('out', 'png', 0.3, 20);
}
